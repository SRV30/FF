...
export const addToWishList = createAsyncThunk(
    ...
    async (productId, { rejectWithValue }) => {
      try {
        const response = await axiosInstance.post("/api/wishList/create", { productId }, { withCredentials: true });
...
export const getWishListItems = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.get("/api/wishList/get", { withCredentials: true });
      return response.data;
    } catch (error) {
...
export const updateWishListItemQty = createAsyncThunk(
  ...
  async ({ _id, qty }, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.put("/api/wishList/update", { _id, qty }, { withCredentials: true });
      return response.data;
    } catch (error) {
      return rejectWithValue(error.response?.data?.message || "Failed to update WishList quantity");
    }
  }
);
...
export const deleteWishListItem = createAsyncThunk(
  ...
  async (_id, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || "Failed to delete WishList item");
    }
  }
);
